...
CREATE INDEX IF NOT EXISTS idx_chat_sessions_age_declined_at
  ON chat_sessions(age_declined_at);
...
export interface AnalyticsContext {
  ...
  ageGateRequired?: boolean | null;
  source?: string | null;
  guidedFlowFilters?: unknown;
}
...
function analyticsSource(analytics?: AnalyticsContext | null): "chat" | "guided_flow" {
  return analytics?.source === "guided_flow" ? "guided_flow" : "chat";
}

function serializeGuidedFlowFilters(value: unknown): string | null {
...
function isCreateTableStatement(statement: string): boolean {
  return statement.toUpperCase().startsWith("CREATE TABLE ");
}
...
async function execSchemaStatement(db: D1Database, statement: string): Promise<void> {
  try {
...
async function ensureSchema(db: D1Database): Promise<void> {
  ...
  const initPromise = (async () => {
    ...
    const remainingStatements = statements.filter((statement) => !isCreateTableStatement(statement));

    for (const statement of tableStatements) {
      await execSchemaStatement(db, statement);
    }

    const migrations = splitSqlStatements(MIGRATION_SQL);
    for (const migration of migrations) {
      try {
        ...
        if (!isDuplicateColumnError(error)) {
          throw new Error(
            `Schema migration failed: ${migration}. Original error: ${
              error instanceof Error ? error.message : String(error)
            }`
          );
        }
...
async function createSearchSequence(
  ...
  const source = analyticsSource(analytics);
  const guidedFlowFilters = source === "guided_flow"
    ? serializeGuidedFlowFilters(analytics?.guidedFlowFilters)
    : null;
...
async function getOrCreateSearchSequence(
  ...
  if (analyticsSource(analytics) === "guided_flow") {
    return createSearchSequence(db, sessionId, messageId, currentIso, userTextRaw, predictedIntent, analytics);
  }
...
export async function recordRecommendationResults(
  ...
  if (recommendations.length > 0) {
    ...
    if (analyticsSource(input.analytics) === "guided_flow") {
      await markSequenceResolved(db, ids.searchSequenceId, currentIso, recommendations[0]?.id ?? null);
    }
  }
  ...
  if (resultCount === 0 && existingSequence?.status === "open") {
    ...
    if (analyticsSource(input.analytics) === "guided_flow") {
      await finalizeSequenceAsUnresolved(db, ids.searchSequenceId, currentIso, "no_results");
    }
  }
...
export async function recordAnalyticsEvent(
  ...
  await ensureSession(
    db,
    {
      sessionId,
      messageId,
      browserToken: safeString(event.browserToken),
      predecessorSessionId: safeString(event.predecessorSessionId),
      sourcePage: null,
      storeId: null,
    },
    occurredAt
...
export async function ensureUser(
  ...
  if (!existing) {
    await db
      ...
      .bind(browserToken, currentIso, currentIso, currentIso, currentIso)
      .run();
  } else {
    await db
      .prepare(
      ...
      .bind(currentIso, currentIso, browserToken)
      .run();
  }
}

export async function recordAgeGateRendered(
  db: D1Database,
  sessionId: string,
  browserToken?: string | null
): Promise<void> {
  ...
  await db
    ...
    .bind(crypto.randomUUID(), sessionId, 'age_gate_rendered', now)
    .run();
}

export async function recordAgeConfirmed(
  db: D1Database,
  sessionId: string,
  browserToken?: string | null
): Promise<void> {
  ...
  await db
    ...
    .bind(crypto.randomUUID(), sessionId, 'age_confirmed', now)
    .run();
  if (browserToken) {
...
export async function recordAgeDeclined(
  db: D1Database,
  sessionId: string,
  browserToken?: string | null
): Promise<void> {
  ...
  await db
    ...
    .bind(crypto.randomUUID(), sessionId, 'age_declined', now)
    .run();
}
