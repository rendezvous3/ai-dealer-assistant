...
// src/index.ts
var app = new Hono2();
var CONFIRMATION_WORDS = ["ok", "okay", "sure", "that one", "this one", "yeah", "yep", "yes"];
var COMPLIANCE_EVENT_TYPES = [
  "age_confirmed",
  "age_gate_rendered",
  "age_declined",
...
var COMPLIANCE_LOG_EVENT_TYPES = COMPLIANCE_EVENT_TYPES.filter((event) => event !== "age_confirmed");
...
__name(rowInt, "rowInt");
function rowFloat(row, key, fallback = 0) {
  const value = row?.[key];
  if (value === null || value === void 0 || value === "") return fallback;
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : fallback;
}
__name(rowFloat, "rowFloat");
...
__name(dateAfter, "dateAfter");
function dateBefore(value) {
  if (!value) return void 0;
  return value.length === 10 ? `${value}T23:59:59.999Z` : value;
}
__name(dateBefore, "dateBefore");
function daysAgoIso(days) {
  const dt = new Date(Date.now() - days * 24 * 60 * 60 * 1e3);
  return dt.toISOString().replace(/\.\d{3}Z$/, "Z");
}
__name(daysAgoIso, "daysAgoIso");
function timeClauses(column, startedAfter, startedBefore) {
  const clauses = [];
...
__name(timeClauses, "timeClauses");
function jsonValue(column, path) {
...
function jsonText(column, path) {
  return `CAST(${jsonValue(column, path)} AS TEXT)`;
}
__name(jsonText, "jsonText");
function inSql(values) {
  return `(${values.map(q).join(", ")})`;
}
__name(inSql, "inSql");
function confirmationExclusionSql() {
  return `m.user_text_normalized NOT IN (${CONFIRMATION_WORDS.map(q).join(", ")})`;
}
__name(confirmationExclusionSql, "confirmationExclusionSql");
function reasonCodeExistsSql(column, code) {
...
__name(reasonCodeExistsSql, "reasonCodeExistsSql");
...
__name(seqWhere, "seqWhere");
function productWhere(params) {
  const clauses = [];
...
__name(productWhere, "productWhere");
function productSort(sortBy, sortDir) {
  const columns = {
    product: "product",
    mentions: "mentions",
    clicks: "clicks",
...
__name(productSort, "productSort");
...
__name(complianceEventWhere, "complianceEventWhere");
...
app.get("/chat-analytics/overview", async (c) => {
  ...
  const bucket = c.req.query("bucket") || void 0;
  const unresolvedOnly = boolParam(c.req.query("unresolved_only"));
  const startedAfter = c.req.query("started_after") || void 0;
  const startedBefore = c.req.query("started_before") || void 0;
  ...
  const bucketJoin = bucket ? ` JOIN chat_search_sequences sq ON sq.search_sequence_id = m.search_sequence_id AND sq.resolved_bucket_label = ${q(bucket)}` : "";
  const totalQueries = hasMessages ? rowInt(
    await first(
      db,
  ...
  const prodAgg = hasProducts ? await first(
    db,
  ...
  const seqAgg = hasSequences ? await first(
    db,
  ...
  const noResultRows = hasSequences ? await all(
    db,
  ...
  const totalSessions = hasSessions ? rowInt(await first(db, `SELECT COUNT(*) AS total_sessions FROM chat_sessions${sessionWhere}`), "total_sessions") : 0;
  const uniqueUsers = hasSessions ? rowInt(await first(db, `SELECT COUNT(DISTINCT browser_token) AS unique_users FROM chat_sessions${sessionWhere}`), "unique_users") : 0;
  ...
  const topBuckets = hasSequences ? (await all(
    db,
  ...
  const refusalAgg = hasEvents ? await first(
    db,
  ...
  const strictRefusalSamples = hasEvents ? (await all(
    db,
...
app.get("/chat-analytics/queries", async (c) => {
  ...
  const offset = intParam(c.req.query("offset"), 0, 0, 1e5);
  const search = c.req.query("search") || void 0;
  const bucket = c.req.query("bucket") || void 0;
  const startedAfter = c.req.query("started_after") || void 0;
  const startedBefore = c.req.query("started_before") || void 0;
...
app.get("/chat-analytics/unresolved", async (c) => {
  ...
  const filter = seqWhere({
    ...
    bucket: c.req.query("bucket") || void 0,
    startedAfter: c.req.query("started_after") || void 0,
    startedBefore: c.req.query("started_before") || void 0,
    unresolvedOnly: true
  });
...
app.get("/chat-analytics/sessions", async (c) => {
  const db = dbFor(c);
  if (!db) return noDb(c);
  if (!await hasTable(db, "chat_sessions")) return c.json({ sessions: [], total: 0 });
  const limit = intParam(c.req.query("limit"), 50, 1, 200);
  const offset = intParam(c.req.query("offset"), 0, 0, 1e5);
  const search = c.req.query("search") || void 0;
  const browserToken = c.req.query("browser_token") || void 0;
  const clauses = [...timeClauses("s.started_at", c.req.query("started_after") || void 0, c.req.query("started_before") || void 0)];
  if (search) {
  ...
  const sessionRows = await all(
    db,
    `SELECT s.session_id, s.message_count, s.sequence_count, s.started_at, s.last_activity_at, s.browser_token, s.predecessor_session_id FROM chat_sessions s${filter} ORDER BY s.started_at DESC LIMIT ${limit} OFFSET ${offset}`
  );
  ...
  const productRows = inList ? await all(
    db,
  ...
  const sequenceRows = inList ? await all(
    db,
...
app.get("/chat-analytics/sessions/:sessionId", async (c) => {
  ...
  const productsByMessage = /* @__PURE__ */ new Map();
  ...
  const events = await hasTable(db, "chat_events") ? (await all(
    db,
  ...
  return c.json({
    ...
    messages: messages.map((message) => {
      ...
      const products = (productsByMessage.get(mid) || []).map((product) => ({
        ...
        shown: product.shown_at !== null && product.shown_at !== void 0,
        clicked: product.clicked_at !== null && product.clicked_at !== void 0,
        external_clicked: product.external_clicked_at !== null && product.external_clicked_at !== void 0
      }));
      return {
        message_id: mid,
...
async function complianceSummaryMetrics(db, startedAfter, startedBefore, sessionColumns) {
  const hasSessions = await hasTable(db, "chat_sessions");
  const hasEvents = await hasTable(db, "chat_events");
  const hasProducts = await hasTable(db, "chat_message_products");
  const columns = sessionColumns || (hasSessions ? await tableColumns(db, "chat_sessions") : /* @__PURE__ */ new Set());
  ...
  const session = hasSessions ? await first(
    db,
  ...
  const event = hasEvents ? await first(
    db,
  ...
  const productWhere2 = where(["p.message_id IS NOT NULL", "p.session_id IS NOT NULL", ...timeClauses("p.shown_at", startedAfter, startedBefore)]);
  const messageWarning = hasProducts ? await first(
    db,
    `WITH recommendation_messages AS (SELECT DISTINCT p.message_id FROM chat_message_products p${productWhere2}) SELECT COUNT(*) AS product_recommendation_messages, SUM(CASE WHEN EXISTS (SELECT 1 FROM chat_events e WHERE e.event_type = 'warning_rendered' AND e.message_id = recommendation_messages.message_id) THEN 1 ELSE 0 END) AS warning_rendered_messages FROM recommendation_messages`
  ) : {};
  const sessionWarning = hasProducts ? await first(
    db,
    `WITH recommendation_sessions AS (SELECT DISTINCT p.session_id FROM chat_message_products p${productWhere2}) SELECT COUNT(*) AS product_recommendation_sessions, SUM(CASE WHEN EXISTS (SELECT 1 FROM chat_events e WHERE e.event_type = 'warning_rendered' AND e.session_id = recommendation_sessions.session_id) THEN 1 ELSE 0 END) AS warning_rendered_sessions FROM recommendation_sessions`
  ) : {};
  const nearHigh = hasEvents ? await first(
    db,
...
__name(complianceSummaryMetrics, "complianceSummaryMetrics");
async function complianceDaily(db, startedAfter, startedBefore, sessionColumns) {
  const hasSessions = await hasTable(db, "chat_sessions");
  const hasEvents = await hasTable(db, "chat_events");
  const hasProducts = await hasTable(db, "chat_message_products");
  const columns = sessionColumns || (hasSessions ? await tableColumns(db, "chat_sessions") : /* @__PURE__ */ new Set());
  ...
  const ensure = /* @__PURE__ */ __name((day) => {
    if (!byDay.has(day)) {
      byDay.set(day, {
        day,
        total_sessions: 0,
  ...
  return [...byDay.entries()].filter(([day]) => Boolean(day)).sort(([a], [b]) => a.localeCompare(b)).map(([, row]) => row);
}
__name(complianceDaily, "complianceDaily");
app.get("/chat-analytics/compliance/summary", async (c) => {
  ...
  return c.json({
    ...
    last_7_days: await complianceSummaryMetrics(db, daysAgoIso(7), void 0, sessionColumns),
    last_30_days: await complianceSummaryMetrics(db, daysAgoIso(30), void 0, sessionColumns),
    total: await complianceSummaryMetrics(db, void 0, void 0, sessionColumns),
...
app.get("/chat-analytics/compliance/events", async (c) => {
  ...
  const filter = complianceEventWhere({
    eventType: c.req.query("event_type") || void 0,
    category: c.req.query("category") || void 0,
    matchedStem: c.req.query("matched_stem") || void 0,
    tier: c.req.query("tier") || void 0,
    layer: c.req.query("layer") || void 0,
    sessionId: c.req.query("session_id") || void 0,
    startedAfter: c.req.query("started_after") || void 0,
    startedBefore: c.req.query("started_before") || void 0
  });
  ...
  return c.json({
    events: rows.map((row) => ({
      ...
      score: row.score === null || row.score === void 0 ? null : rowFloat(row, "score"),
      replacement_count: row.replacement_count === null || row.replacement_count === void 0 ? null : rowInt(row, "replacement_count"),
...
app.get("/chat-analytics/compliance/filters", async (c) => {
  ...
  const base = [`e.event_type IN ${inSql(COMPLIANCE_LOG_EVENT_TYPES)}`, ...timeClauses("e.occurred_at", c.req.query("started_after") || void 0, c.req.query("started_before") || void 0)];
  async function distinctJson(path) {
  ...
  __name(distinctJson, "distinctJson");
...
app.get("/chat-analytics/compliance/tuning", async (c) => {
  ...
  return c.json({
    ...
    near_misses_by_nearest_stem: nearRows.map((row) => ({ nearest_stem: String(row.nearest_stem || "unknown"), event_count: rowInt(row, "event_count"), message_count: rowInt(row, "message_count"), avg_score: row.avg_score === null || row.avg_score === void 0 ? null : rowFloat(row, "avg_score") })),
...
app.get("/chat-analytics/guided-flow", async (c) => {
  ...
  if (!db) return noDb(c);
  const hasSequences = await hasTable(db, "chat_search_sequences");
  if (!hasSequences) return c.json({ sequences: [], total: 0, query_summary: [] });
  const limit = intParam(c.req.query("limit"), 50, 1, 200);
  const offset = intParam(c.req.query("offset"), 0, 0, 1e5);
  const search = c.req.query("search") || void 0;
  const startedAfter = c.req.query("started_after") || void 0;
  const startedBefore = c.req.query("started_before") || void 0;
  const clauses = ["source = 'guided_flow'"];
  ...
  const sequences = await all(
    db,
    `SELECT search_sequence_id, session_id, resolved_query_text, resolved_query_normalized, status, cue_verdict, intent_verdict, recommendation_verdict, guided_flow_filters, started_at, ended_at FROM chat_search_sequences${filter} ORDER BY started_at DESC LIMIT ${limit} OFFSET ${offset}`
  );
  const querySummary = await all(
    db,
    `SELECT COALESCE(resolved_query_normalized, resolved_query_text, 'unknown') AS query, COUNT(*) AS count FROM chat_search_sequences${filter} GROUP BY query ORDER BY count DESC LIMIT 20`
  );
  return c.json({
