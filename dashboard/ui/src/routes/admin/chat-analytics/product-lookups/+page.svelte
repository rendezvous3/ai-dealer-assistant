<script lang="ts">
	...
	async function fetchData() {
		...
		try {
			const res = await fetch(`/admin/chat-analytics/_api/product-lookups?${params}`);
	...
	function applyFilters(overrides: Record<string, string> = {}) {
		...
		p.set('offset', '0');
		goto(`/admin/chat-analytics/product-lookups?${p}`);
	}
...
<DashboardShellV3 pageTitle="Chat Analytics">
	...
	<div class="mb-6 flex flex-wrap gap-1 border-b border-white/[0.07]">
		...
		<div class="border-b-2 border-white px-4 py-2 text-sm font-medium text-white">Vehicle Lookups</div>
	...
	<div class="mb-6 flex flex-wrap items-center gap-3">
		<input type="text" placeholder="Search vehicle lookups…" bind:value={searchInput} on:keydown={(e) => { if (e.key === 'Enter') applyFilters(); }} class="h-8 w-64 rounded-lg border border-white/[0.07] bg-[#141414] px-3 text-xs text-[#f5f5f5] placeholder-[#525252] outline-none focus:border-white/[0.15]" />
		...
		<div class="overflow-x-auto">
			<table class="w-full text-xs">
				<thead><tr class="border-b border-white/[0.05]">
					<th class="px-4 py-2.5 text-left font-medium tracking-widest text-[#525252] uppercase">Vehicle</th>
					<th class="px-3 py-2.5 text-left font-medium tracking-widest text-[#525252] uppercase">Category</th>
					<th class="px-3 py-2.5 text-right font-medium tracking-widest text-[#525252] uppercase">Lookups</th>
					<th class="px-3 py-2.5 text-right font-medium tracking-widest text-[#525252] uppercase">Follow-Ups</th>
					<th class="px-3 py-2.5 text-right font-medium tracking-widest text-[#525252] uppercase">No Match</th>
					<th class="px-3 py-2.5 text-right font-medium tracking-widest text-[#525252] uppercase">Clarify</th>
				...
				<tbody class="divide-y divide-white/[0.04]">
					{#if loading}
						<tr><td colspan="8" class="px-5 py-12 text-center text-sm text-[#737373]">Loading vehicle lookups…</td></tr>
					{:else if rows.length === 0}
						<tr><td colspan="8" class="px-5 py-12 text-center text-sm text-[#737373]">No lookup data for this range.</td></tr>
					{:else}
						{#each rows as row}
							<tr class="transition hover:bg-white/[0.02]">
								<td class="px-4 py-3 text-[#f5f5f5]">{row.product}</td>
								<td class="px-3 py-3 text-[#737373]">{row.category || row.make || '—'}</td>
								<td class="px-3 py-3 text-right text-[#f5f5f5]">{row.lookup_count}</td>
								<td class="px-3 py-3 text-right text-[#737373]">{row.follow_up_count}</td>
								<td class="px-3 py-3 text-right text-[#737373]">{row.no_match_count}</td>
								<td class="px-3 py-3 text-right text-[#737373]">{row.clarification_count}</td>
